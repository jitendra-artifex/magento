...
try {
    $installer->run("SELECT * FROM {$this->getTable('rw_youtube_videos')}");
} catch (Exception $e) {
    $installer->run("
...
(
    ...
    c.store_id, 
    c.value as video_code, 
    d.value as video_title, 
    e.value as video_thumb_width, 
    f.value as video_thumb_height, 
    g.value as video_width, 
    h.value as video_height
    FROM
...
$installer->run("ALTER IGNORE TABLE {$this->getTable('rw_youtube_videos')}
	ADD COLUMN video_host INT NOT NULL DEFAULT '1' AFTER video_height;");
