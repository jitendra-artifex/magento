...
class AW_Helpdeskultimate_Helper_Notify extends AW_Helpdeskultimate_Helper_Abstract
{
    ...
    public function ticketReassigned($ticket)
    {
        ...
        if(Mage::getStoreConfig('helpdeskultimate/advanced/email_customer_reassign', $ticket->getStoreId()) && $mailTemplate) {               
            $mailTemplate
                ->setDesignConfig(array('area' => 'backend'))
                ->sendTransactional(
                    $newDepartment->getToCustomerReassignEmail() ? $newDepartment->getToCustomerReassignEmail() : Mage::getStoreConfig('helpdeskultimate/generaldep/to_customer_reassign_email', $ticket->getStoreId()),
        ...
        if ($newDepartment->getEnabled() && $newDepartment->getNotify()) { 
            ...
            if (!is_null($mailTemplate)) {
                ...
                $mailTemplate
                    ->setDesignConfig(array('area' => 'backend'))
                    ->sendTransactional(
                        $newDepartment->getToAdminReassignEmail() ? $newDepartment->getToAdminReassignEmail() : Mage::getStoreConfig('helpdeskultimate/generaldep/to_admin_reassign_email', $ticket->getStoreId()),
                        $newDepartment->getSender(),
                        $recipientEmail,
                        $recipientName,
