...
class AW_Helpdeskultimate_TicketController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function saveAction()
    {
        try {
            ...
            if ($data = $this->getRequest()->getPost()) {
                ...
                if ($ticket->isReadOnly()) {
                    throw(new Exception($this->__("Ticket has been locked by %s at %s", $ticket->getLockedUser()->getName(), $ticket->getLockedAt())));
                }
                if (isset($data['store_id'])) {
                    $ticket->setStoreId($data['store_id']);
                }
    ...
    private function _createUploader()
    {
        ...
        foreach ($_FILES as $key => $file) {
            if (isset($file['name']) && !empty($file['name'])) {
                if ($file['size'] > ($this->_helper('config')->getAttachmentsMaxUploadFileSize() * self::SIZE_MEGABYTE) || $file['error'] === UPLOAD_ERR_INI_SIZE) {
    ...
    {
        $department = Mage::helper('helpdeskultimate')->getDepartment($this->getRequest()->getParam('department_saved_id', 0));
