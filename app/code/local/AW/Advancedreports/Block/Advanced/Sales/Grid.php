...
class AW_Advancedreports_Block_Advanced_Sales_Grid extends AW_Advancedreports_Block_Advanced_Grid
{
    ...
    public function addOrderItems()
    {
		...
		$this->getCollection()->getSelect()
                ...
                ->columns(array('base_row_xrefunded'=>"(IF((item.qty_refunded > 0), 1, 0) * item.base_row_invoiced)"))
    ...
    public function _prepareCollection()
    {
        ...
        if ($this->_checkVer('1.7.1.0') && !$this->_checkVer('1.8.0.0')){
            $this->_prepareAddressesFor1710($date_from, $date_to, (isset($storeIds) ? $storeIds : null));
        }
        # End of getting data
...
//		echo $this->getCollection()->getSelect()->__toString();
		foreach ($this->getCollection() as $item)		
		{
            ...
            $row['base_row_subtotal'] = $row['qty_ordered'] * $row['base_price'];

            $row['base_row_total'] = ($row['base_row_subtotal'] + $row['base_tax_amount'] - abs($row['base_discount_amount']));

            if ($row['base_row_invoiced'] > $row['base_row_total']){
                $row['base_row_invoiced'] = $row['base_row_total'];
            }
            if ($row['base_row_xrefunded'] > $row['base_row_invoiced']){
                $row['base_row_xrefunded'] = $row['base_row_invoiced'];
            }
